...
async function startServer() {
  ...
  app.post("/api/hina", async (req, res) => {
    ...
    const catalogText = Array.isArray(catalog) ? `\nHidden India destination catalog (use only these destination names for recommendations):\n${catalog.map((item: { name: string; state: string; description: string; bestTime: string; activities?: string[] }) => `- ${item.name} — ${item.state}; ${item.description}; best time: ${item.bestTime}; activities: ${(item.activities || []).join(", ")}`).join("\n")}` : "";
